...
import { color2select } from "@/actions/image";
export function Colorpage_30() {
    ...
    const next = () => {  
        if(select)  
            navigate("/colorpage_10");
            const value = `${select}`;
            dispatch(color2select(value));
            console.log('color2', imgkey);
    }
    console.log(select)
    return (
        <div className="flex flex-col items-center relative mt-16">
            <div className="flex flex-col relative items-center">
                <p className="text-[24px] font-bold text-center">What color do you like for the design ? (%30)</p>
                <p className="text-[15px] font-normal text-center">Hint: This color will cover 30% of the available area in your room.<br></br> area in your room.</p>
            </div>
                    ...
                    {color_30.map((item) => (
                        <div>
                            <div className="flex flex-col items-center">
                                {/* <button onClick={()=>setSelect(`${item.bgcolor}`)} className={`bg-[${item.bgcolor}]  h-12 w-14 focus:brightness-50 hover:brightness-75 active:opacity-100 rounded-t-full rounded-b-full`}> */}
